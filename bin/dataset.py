...
class BasicSmilesDataset(Dataset):
    ...
    def encode(self, smiles):
        ...
        max_seqlen = max([len(smi) for smi in smiles])
...
class TrainingSmilesDataset(BasicSmilesDataset):
    def __init__(self, filepath, filepath_char2idx=None, device=None):
        ...
        ## Read a file
        self.df = pd.read_csv(filepath_smiles, header=None).rename(columns={0:"smiles_src", 1:"smiles_tar", 2:"smiles_neg"})
            ...
            for smi in smiles:
                for char in smi:
                    if char not in self.char2idx:
                        self.char2idx[char] = t
                        self.idx2char.append(char)
                        t += 1
        return char2idx, idx2char


class SmilesValidationDataset(BasicSmilesDataset):
    def __init__(self, filepath, filepath_char2idx, device=None):
        ...
        device : (optional) Location for PyTorch neural network. Default is CPU
        """
        super(SmilesValidationDataset, self).__init__(device)
        ## Read a file
        self.df = pd.read_csv(filepath_smiles, header=None).rename(columns={0:"smiles_src"})
        self.num_smiles = len(self.df)
        ## Construct vocabulary
        self.char2idx, self.idx2char = self._load_vocab(filepath_char2idx)
        self.vocab_size = len(self.char2idx)
